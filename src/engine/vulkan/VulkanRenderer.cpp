...
VulkanRenderer::VulkanRenderer(const ApplicationState* applicationState, const leoscene::Camera* camera) :
...
void VulkanRenderer::_cleanupSwapChainDependentObjects()
{
    vkDeviceWaitIdle(_device);
...
void VulkanRenderer::_recreateSwapChainDependentObjects()
{
...
void VulkanRenderer::init(GLFWwindow* window)
{
    ...
    */
    ...
    MaterialBuilder::Parameters materialBuilderParameters = {};
    materialBuilderParameters.device = _device;
    materialBuilderParameters.instance = &_vulkan;
    materialBuilderParameters.multisamplingNbSamples = instanceProperties.maxNbMsaaSamples;
    materialBuilderParameters.forwardRenderPass = _renderPass;
    _materialBuilder.init(materialBuilderParameters);
...
void VulkanRenderer::drawFrame()
{
    ...
    if (result == VK_ERROR_OUT_OF_DATE_KHR) {
        _cleanupSwapChainDependentObjects();
        _recreateSwapChainDependentObjects();
    }
...
void VulkanRenderer::loadSceneToRenderer(const leoscene::Scene* scene)
{
    /*
...
void VulkanRenderer::_createGlobalDescriptors(uint32_t _totalInstancesNb)
{
    ...
    _globalDescriptorAllocator.init(_device, globalDescriptorAllocatorOptions);
...
void VulkanRenderer::_createCullingDescriptors(uint32_t nbObjects)
{
    ...
    _cullingDescriptorAllocator.init(_device, cullingDescriptorAllocatorOptions);
...
void VulkanRenderer::_createDepthPyramidDescriptors()
{
    ...
    _depthPyramidDescriptorAllocator.init(_device, depthPyramidDescriptorAllocatorOptions);
